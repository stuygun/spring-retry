...
public class AmazonService {
    ...
    @Recover
    public String recover(RuntimeException runtimeException, MultipartFile multipartFile) throws Exception {
        log.error(runtimeException.getMessage(), runtimeException);
        try {
            File file = convertMultiPartToFile(multipartFile);
            log.info("File has been saved locally due to S3 connectivity issues.");
            return "Locally Stored, not to S3!";
        } catch (Exception exception){
            log.error(exception.getMessage(), exception);
            log.error("File even couldn't be stored locally!");
            throw exception;
        }
    }
    ...
    public String uploadFile(MultipartFile multipartFile) throws Exception {
        ...
        try {
            ...
            if (file != null) {
                ...
                log.debug("File(name:{}) delete operation successful(result={}).", file.getName(), deleteResult);
            }
            throw exception;
        }
